...
def coletar_dados_adicionais():
    ...
    numEquip = int(input('Qual a potência elétrica (em W) somada de todos os equipamentos da sala? '))
    print('Dados armazenados\n')
    return numPessoas, numEquip


def calcular_carga_termica(volume, posicao, areaJanela, areaPorta, numPessoas, numEquip):
    # Tabela 1
    if posicao == 1:
        recinto = volume * 16
    else:
...
def exibir_resultados(recinto, janela, porta, pessoas, aparelhos, cargaTermica, cargaBTU):
...
def main():
    ...
    recinto, janela, porta, pessoas, aparelhos, cargaTermica, cargaBTU = calcular_carga_termica(
